                  ...
                  <div className="p-2 space-y-1 max-h-[60vh] overflow-y-auto">
                    {practiceSections.map((section, idx) => (
                      <button
                        key={idx}
                        onClick={() => {
                          const el = document.getElementById(`practice-section-${idx}`);
                          if (el) el.scrollIntoView({ behavior: 'smooth' });
                        }}
                        className="w-full text-left px-3 py-2 text-sm text-foreground hover:bg-indigo-50 hover:text-indigo-700 dark:hover:bg-indigo-900/30 rounded-lg transition-colors font-medium truncate"
